...
!
    ...
    REAL(fp) :: d0, kappa, eps, s1, s2, lambda, deltaH
    INTEGER                :: count
    ...
    DO WHILE ( ABS(ipH-newpH) .gt. MINVAL )
       ...
       ! Apply Newton's method
           ! parameters
        eps = 1.0e-10_fp   ! small number to prevent division by zero

        ! compute candidate Newton step
        !deltaH = f / df

        ! compute dimensionless measures
        !s2 =  abs(f) / (abs(df) * nHPLUS + eps)

        ! compute smooth damping factor
        !lambda = 1.0_fp / ( 1.0_fp  + s2 )

        ! apply adaptive damped Newton step
        ! nnHPLUS = nHPLUS - lambda * deltaH
        nnHPLUS = nHPLUS - 1.0_fp / ( 1.0_fp  +  abs(f) / (abs(df) * nHPLUS + &
                  eps) ) * f/df
        ! safeguard against negative H (optional, usually H>0 anyway)
        IF (nnHPLUS .le. 0.0_fp) THEN
           nnHPLUS = 1.0e-14_fp
        ENDIF
