...
#ifndef NO_OMP
             CALL Timer_Start  ( "=> Sulfur chem", RC, .TRUE., Thread)
       !      WRITE(6,'(a,i3)') '>>> SULFUR TIMER START IN THREAD ', THREAD
#else
             CALL Timer_Start  ( "=> Sulfur chem", RC)
       !      WRITE(6,*) '>>> SULFUR TIMER START <<<'
#endif
          ENDIF

       ! Calculate cloud pH
       CALL GET_HPLUS( SO4nss, HMSc, TNH3, TNO3,  SO2_AfterSS,   CL, TNA, TDCA, &
                       TFA,    TAA,  TK,   PATM,  LWC, HPLUS_45, HPLUS, iter_count1, &
                       iter_count2)

          IF ( Input_Opt%useTimers ) THEN
#ifndef NO_OMP
             CALL Timer_End( "=> Sulfur chem", RC, .TRUE., Thread)
       !      WRITE(6,'(a,i3)') '>>> SULFUR TIMER END IN THREAD ', THREAD
#else
             CALL Timer_End  ( "=> Sulfur chem", RC)
        !     WRITE(6,*) '>>> SULFUR TIMER END <<<'
#endif
          ENDIF

       ! Store the cloud pH quantities
       State_Chm%isCloud(I,J,L)    = REAL(iter_count2, fp)
       State_Chm%pHCloud(I,J,L)    = -1.0_fp * log10(HPLUS)
       State_Chm%QLxpHCloud(I,J,L) = REAL(iter_count1, fp)
...
!
    SUBROUTINE GET_HPLUS( SO4nss, HMSc, TNH3, TNO3, SO2, CL, TNA, TDCA, TFA, &
                          TAA,  T, PRES, LWC,  iHPLUS, HPLUS, iter_count1, &
                          iter_count2 )
!
! !USES:
!
...
!
    ...
    INTEGER,  INTENT(OUT)   :: iter_count1
    INTEGER,  INTENT(OUT)   :: iter_count2
...
!
    ...
    REAL(fp)               :: f, df, nnHPLUS, fCa, dCa
        REAL(fp)               :: afCa, bfCa, adCa, bdCa
	REAL(fp)               :: apH, bpH, cpH, FTST
	REAL(fp)               :: aHPLUS, bHPLUS
	REAL(fp)               :: HA, HB, FA, FB
    REAL(fp)               :: afHCO3, afCO3
    REAL(fp)               :: afHSO3, afSO3
    REAL(fp)               :: afHNO3, afNH4, afHCl
    REAL(fp)               :: afAA, afFA
    REAL(fp)               :: bfHCO3, bfCO3
    REAL(fp)               :: bfHSO3, bfSO3
    REAL(fp)               :: bfHNO3, bfNH4, bfHCl
    REAL(fp)               :: bfAA,   bfFA
    ...
    DO WHILE ( ABS(apH-cpH) .gt. MINVAL )

       COUNT = COUNT+1
       bpH = (apH + cpH) / 2.0
	   
       aHPLUS = 10.e+0_fp**(-apH)
	   bHPLUS = 10.e+0_fp**(-bpH)

       ! Get f(x) terms
       afHCO3  = kCO21 ( PRES, T, LWC, aHPLUS )

       afCO3 = kCO22 ( PRES, T, LWC, aHPLUS )

       afHSO3  = kSO21 ( PRES, T, LWC, aHPLUS, SO2 )

       afSO3 = kSO22 ( PRES, T, LWC, aHPLUS, SO2 )

       afHNO3 = kHNO3 ( PRES, T, LWC, aHPLUS, TNO3 )

       afNH4  = kNH3  ( PRES, T, LWC, aHPLUS, TNH3, Kw_T )

       ! include HCl in cloud pH calculations, xnw 10/17/17
       afHCl  = kHCl  ( PRES, T, LWC, aHPLUS, CL  )

       afFA   = kFA   ( PRES, T, LWC, aHPLUS, TFA ) ! jmm 12/3/18

       afAA   = kAA   ( PRES, T, LWC, aHPLUS, TAA ) ! jmm 12/3/18

      
       ! Calculate [Ca2+] in equilibrium with CaCO3(s)
       CALL CaCO3_PRECIP ( PRES, T, aHPLUS, afCa, adCa )

       ! if [Ca2+] in equilibrium with CacO3(s) is greater than total [Ca2+]
       ! then all Ca is dissolved else [Ca2+] varies with [H+]
       IF ( afCa .ge. TDCA ) THEN
          ! Non-volatile aerosol concentration [M]
          D = (2.e+0_fp*SO4nss) - (TNA+2.e+0_fp*TDCA)

          ! Define f(x)
          FA = D - aHPLUS + Kw/aHPLUS + afHCO3 + 2.e+0_fp * &
               afCO3 + afHSO3 + 2.e+0_fp * afSO3 + afHNO3 - afNH4 + &
               afHCl + afFA + afAA
       ...
       ELSE
          ...
          ! Define f(x)
          FA = D - aHPLUS + Kw/aHPLUS + afHCO3 + 2.e+0_fp * afCO3 + &
               afHSO3 + 2.e+0_fp * afSO3 + afHNO3 - afNH4 + &
               afHCl + afFA + afAA - 2.e+0_fp * afCa

       ENDIF

       ! Get f(x) terms
       bfHCO3  = kCO21 ( PRES, T, LWC, bHPLUS )

       bfCO3 = kCO22 ( PRES, T, LWC, bHPLUS )

       bfHSO3  = kSO21 ( PRES, T, LWC, bHPLUS, SO2 )

       bfSO3 = kSO22 ( PRES, T, LWC, bHPLUS, SO2 )

       bfHNO3 = kHNO3 ( PRES, T, LWC, bHPLUS, TNO3 )

       bfNH4  = kNH3  ( PRES, T, LWC, bHPLUS, TNH3, Kw_T )

       ! include HCl in cloud pH calculations, xnw 10/17/17
       bfHCl  = kHCl  ( PRES, T, LWC, bHPLUS, CL  )

       bfFA   = kFA   ( PRES, T, LWC, bHPLUS, TFA ) ! jmm 12/3/18

       bfAA   = kAA   ( PRES, T, LWC, bHPLUS, TAA ) ! jmm 12/3/18

      
       ! Calculate [Ca2+] in equilibrium with CaCO3(s)
       CALL CaCO3_PRECIP ( PRES, T, bHPLUS, bfCa, bdCa )

       ! if [Ca2+] in equilibrium with CacO3(s) is greater than total [Ca2+]
       ! then all Ca is dissolved else [Ca2+] varies with [H+]
       IF ( bfCa .ge. TDCA ) THEN
          ! Non-volatile aerosol concentration [M]
          D = (2.e+0_fp*SO4nss) - (TNA+2.e+0_fp*TDCA)

          ! Define f(x)
          FB = D - bHPLUS + Kw/bHPLUS + bfHCO3 + 2.e+0_fp * &
               bfCO3 + bfHSO3 + 2.e+0_fp * bfSO3 + bfHNO3 - bfNH4 + &
               bfHCl + bfFA + bfAA
       ...
       ELSE
          ...
          ! Define f(x)
          FB = D - bHPLUS + Kw/bHPLUS + bfHCO3 + 2.e+0_fp * bfCO3 + &
               bfHSO3 + 2.e+0_fp * bfSO3 + bfHNO3 - bfNH4 + &
               bfHCl + bfFA + bfAA - 2.e+0_fp * bfCa
